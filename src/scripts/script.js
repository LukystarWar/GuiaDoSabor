...
async function loadData() {
    try {
        ...
        placesGrid.innerHTML = '<p style="text-align: center; color: #6C757D;">Erro ao carregar os dados.</p>';
    }
}
...
function renderFilterChips() {
    filterChips.innerHTML = categories.map(cat => `
        <button
            class="filter-chip ${cat.id === currentCategory ? 'filter-chip--active' : ''}"
...
function renderPlaces() {
    ...
    placesGrid.innerHTML = filtered.map(place => {
        const categoryName = categories.find(c => c.id === place.category)?.name || place.category;

        return `
            <div class="place-card ${place.hasReview ? 'place-card--clickable' : ''}" ${place.hasReview ? `onclick="window.location.href='lugar.html?slug=${place.reviewSlug}'"` : ''}>
                ...
                <div class="place-card__header">
                    ...
                    <div class="place-card__info">
                        ...
                        <span class="place-card__category">${categoryName}</span>
                    </div>
                </div>
